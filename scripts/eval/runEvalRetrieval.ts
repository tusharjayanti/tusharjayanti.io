...
// are diff-able.
//
// retrieval.ts is the script entrypoint that invokes this function;
// the dispatch primitives (dispatchQuery, computeFailureRate) live in
// dispatch.ts and stay side-effect-free on import so tests can use
// them without triggering this runner.
...
import { HAIKU_MODEL } from '../../api/_reranker.js';
import {
  writeResult,
  loadBaseline,
  gatherEnvMetadata,
...
export async function runEvalRetrieval(): Promise<void> {
